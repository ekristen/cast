...
A version 2 cast distro is one like [example-distro-saltstack](https://github.com/ekristen/example-distro-saltstack) where the salt states start in the root of the project and the release is generated by the `cast-devtools`. The reason for moving the states to the root is for `git submodule` usage for distros that extend another distro.
